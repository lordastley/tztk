...
my %packet; %packet = (
  ...
  cs_login => sub { #(user, pass)
    ...
    chr(0x01) . $packet{cs_long}($protocol_version)  . $packet{cs_string}($_[0])  . ("\0" x 16); #This might be right post beta 1.4. Probably need to modify total length. -t9 9/15/11
  },
...
$server_pid = open2(\*MCOUT, \*MCIN, "java -server -XX:+UseConcMarkSweepGC -XX:ParallelGCThreads=8 -XX:+UseAdaptiveGCBoundary -XX:-UseGCOverheadLimit -XX:SurvivorRatio=16 -Xnoclassgc -XX:UseSSE=3 -Xmx$server_memory -Xms$server_memory -jar minecraft_server.jar nogui 2>&1");
  ...
  foreach my $fh ($sel->can_read(60)) {
    if ($fh == \*STDIN) {
      ...
      if (defined $cmd_name && command_allowed($cmd_name)) {
        ...
        if (($cmd_name eq 'create' || $cmd_name eq 'c') && $cmd_args =~ /^(\d+)(?:\s*\D+?\s*(\d+))?|([a-z][\w\-]*)$/) {
          ...
          show_help();
	} elsif($cmd_name eq 'mode' && $cmd_args =~ /^(\d)$/) { #lemme try hacking in the new gamemode command from 1.8 -t9
