...
# lordastley - and more modifications from: https://github.com/lordastley/tztk
...
#$server_pid = open2(\*MCOUT, \*MCIN, "java -server -XX:+UseConcMarkSweepGC -XX:ParallelGCThreads=8 -XX:+UseAdaptiveGCBoundary -XX:-UseGCOverheadLimit -XX:SurvivorRatio=16 -Xnoclassgc -XX:UseSSE=3 -Xmx$server_memory -Xms$server_memory -jar minecraft_server.jar nogui 2>&1"); #lordastley - personal modifications.
MCOUT->blocking(0);
    ...
    if ($fh == \*STDIN) {
      ...
      while (my $mc = <MCOUT>) {
        ...
        if (defined $cmd_name && command_allowed($cmd_name)) {
          ...
          # lordastley - alias create to c, retain both.
          ...
          } elsif($cmd_name eq 'mode') {
            lordastley_mode( $cmd_user, $cmd_args);
          }
        }
      } #/while(mcout)
